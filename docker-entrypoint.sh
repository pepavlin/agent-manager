...
# Fix volume permissions (runs as root, volumes may be owned by root)
chown -R agent:nodejs /app/data
chown -R agent:nodejs /home/agent

# Ensure upload directory exists
mkdir -p "${STORAGE_PATH:-/app/data/uploads}"
chown -R agent:nodejs "${STORAGE_PATH:-/app/data/uploads}"

# Setup Claude credentials as the agent user
...
CREDENTIALS_EOF
  ...
  chown agent:nodejs "$CLAUDE_DIR/.credentials.json"
...
if [ ! -f "/home/agent/.claude.json" ]; then
  CLAUDE_VERSION=$(su-exec agent claude --version 2>/dev/null || echo "1.0.0")

  cat > "/home/agent/.claude.json" << ONBOARDING_EOF
{
...
ONBOARDING_EOF
  ...
  chown agent:nodejs "/home/agent/.claude.json"
...
# Run database migrations (as agent user)
echo "Running database migrations..."
su-exec agent npx prisma migrate deploy
echo "Migrations complete."

# Drop to agent user and execute the main command
exec su-exec agent "$@"
